...
# To merge candidate loops that are within max_dist bins of each other
# This is applied following the binary matrix, so it does not 
# remove the binary matrix's impact.
#
# Returns the loops with the lowest p-value that are within max_dist bins of each other
def merge_scales(candidates, max_dist=2):

    clusters = []

    for candidate in candidates:
        y, x, s, score, pval = candidate

        assigned = False

        for cluster in clusters:

            # Compare to representative point of cluster
            cy, cx = cluster[0][0], cluster[0][1]

            if abs(y - cy) <= max_dist and abs(x - cx) <= max_dist:
    ...
    for cluster in clusters:
        best = min(cluster, key=lambda v: v[4])   # Lowest p-value
        merged.append(best)
...
def apa(raw_hic_numpy, final_loops): # Aggregate Peak Analysis
    ...
    for y, x, sigma, score, p, q in final_loops:
...
file_path = project_root / out_dir / f"loops_{chr1}_{chr2}_{res}_{pT}_{kern}.csv"
...
for i in range(chr1, chr2+1):
    ...
    for r_start in range(0, shape[0], step):
        ...
        for c_start in range(0, shape[1], step):
            ...
            # loc_max_matrix = local_2d_max(response_volume)
            # keep = consec_scale_test(loc_max_matrix)
    ...
    #Merging loops with max_dist bins. Done to take care of overlap
    # print("Merging loops within 2 bins...")
    # bin_rep = merge_scales(bin_rep)
    ...
    all_apa.append((chrom_it, apa(raw_hic_numpy, final_loops)))
    ...
    if compare:
        ...
        for b in final_loops:
            ...
            for m in m_loops:
                ...
                if abs(m[1] - b[0]) <= 5 and abs(m[2] - b[1]) <= 5:
                    match.append((chrom_it,) + b)
                    all_match.append((chrom_it,) + b)
                    found = True
                    break

            if not found:
                no_match.append((chrom_it,) + b)
...
with open(file_path, "w", newline="") as f:
    ...
    writer.writerow([f"All Loops Found: {len(all_loops)}"])
    writer.writerow(["Chromosome", " bin1", " bin2", " sigma", " DoH score", " p-value", " q"])
    writer.writerows(all_loops)

    writer.writerow([])  

    if compare:
        writer.writerow([f"Loops Overlapping with MUSTACHE: {len(all_match)}"])
        writer.writerow([f"Total # of MUSTACHE loops: {len(m_loops)}"])
        writer.writerow(["Chromosome", " bin1", " bin2", " sigma", " DoH score", " p-value", " q"])
        writer.writerows(all_match)

        writer.writerow([])

        writer.writerow([f"Loops not overlapping: {len(no_match)}"])
        writer.writerow(["Chromosome", " bin1", " bin2", " sigma", " DoH score", " p-value", " q"])
    ...
    writer.writerow(["APA scores per chromosome:"])
    writer.writerow(["Chromosome", " APA Score"])
    writer.writerows(all_apa)
...
for i in range(chr1,chr2+1):
    ...
    print(f"\nChromosome {chrom_it}")
    print(f"Best window contains {best_count} loops")
